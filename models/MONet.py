...
class MONetModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument(
            ...
            type=int,
            default=11,
            help="Number of supported slots",
        )
        parser.add_argument(
            "--z_dim",
            type=int,
            default=16,
            help="Dimension of individual z latent per slot",
        )
        parser.add_argument(
            "--image_size",
            type=int,
        ...
        if is_train:
            parser.add_argument(
                "--beta", type=float, default=0.5, help="weight for the encoder KLD"
            )
            parser.add_argument(
                "--gamma", type=float, default=0.5, help="weight for the mask KLD"
            )
        return parser
    ...
    def __init__(self, opt):
        ...
        self.visual_names = (
            ["m{}".format(i) for i in range(opt.num_slots)]
            + ["x{}".format(i) for i in range(opt.num_slots)]
            + ["xm{}".format(i) for i in range(opt.num_slots)]
            + ["x", "x_tilde"]
        )
        self.model_names = ["Attn", "CVAE"]
        self.netAttn = networks.init_net(
            networks.Attention(opt.input_nc, 1, image_size=opt.image_size),
            gpu_ids=self.gpu_ids,
        )
        self.netCVAE = networks.init_net(
            networks.ComponentVAE(opt.input_nc, opt.z_dim, image_size=opt.image_size),
            gpu_ids=self.gpu_ids,
        )
        ...
        if self.isTrain:  # only defined during training time
            ...
            self.optimizer = optim.RMSprop(
                chain(self.netAttn.parameters(), self.netCVAE.parameters()), lr=opt.lr
            )
    ...
    def forward(self):
        ...
        for k in range(self.opt.num_slots):
            ...
            m_tilde_k_logits, x_mu_k, x_logvar_k, z_mu_k, z_logvar_k = self.netCVAE(
                self.x, log_m_k
            )
            x_logvar_k = x_logvar_k.mean() # dirty hack for dataparallel doubling the scalar size
            # KLD is additive for independent distributions
            self.loss_E += (
                -0.5 * (1 + z_logvar_k - z_mu_k.pow(2) - z_logvar_k.exp()).sum()
            )
            ...
            b_k = (
                log_m_k
                - 0.5 * x_logvar_k
                - (self.x - x_mu_k).pow(2) / (2 * x_logvar_k.exp())
            )
            b.append(b_k.unsqueeze(1))

            # Get outputs for kth step
            setattr(
                self, "m{}".format(k), m_k * 2.0 - 1.0
            )  # shift mask from [0, 1] to [-1, 1]
    ...
    def backward(self):
        ...
        self.loss_mask = self.criterionKL(
            self.m_tilde_logits.log_softmax(dim=1), self.m
        )
        loss = (
            self.loss_D + self.opt.beta * self.loss_E + self.opt.gamma * self.loss_mask
        )
        loss.backward()

...
class ComponentVAE(nn.Module):
    def __init__(self, input_nc, z_dim=16, image_size=64):
        super().__init__()
        self._input_nc = input_nc
        self._z_dim = z_dim
        # full_res = False # full res: 128x128, low res: 64x64
        h_dim = int(image_size ** 2 / 4)
        ...
        self._bg_logvar = torch.tensor(0.09).log().to(device)
        self._fg_logvar = torch.tensor(0.11).log().to(device)
...
class Attention(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, image_size=64, ngf=64):
        """Construct a Unet generator
        Parameters:
            input_nc (int)  -- the number of channels in input images
            output_nc (int) -- the number of channels in output images
        ...
        downblock_output_size = int((image_size / 2 ** 4) ** 2 * 8 * ngf)
        self.mlp = nn.Sequential(
            nn.Linear(downblock_output_size, 128),
            nn.ReLU(),
            nn.Linear(128, 128),
            nn.ReLU(),
            nn.Linear(128, downblock_output_size),
            nn.ReLU(),
        )
        ...
        self.upblock5 = AttentionBlock(ngf * 4, ngf)
        # no resizing occurs in the last block of each path
